...
class NewFunctions:
    """
    A class that handles the operation related to changing the definition of
    the involved functions so that the external interfaces of the involved
    functions remain the same, and they call the block function with the correct
    ...
    def change_function_definitions(involved_functions, block_call_union):
        ...
        so that the code that calls these function will not be affected by the
        tail call elimination and the external interface of the functions
        remain the same.
        """
        block_call_union_instance = Block.generate_block_call_union_instance(
            block_call_union
        )
        for function in involved_functions:
            block_items = [block_call_union_instance]
            NewFunctions.generate_params_assignments_in_frame(
                block_items, involved_functions[function]
            )
            block_items.append(
                Block.generate_block_call_stmt(
                    involved_functions[function].index_label
                )
            )
            block_items.append(
                NewFunctions.generate_return_stmt_in_new_functions(function)
            )
    ...
    def generate_params_assignments_in_frame(block_items, function_info):
        ...
        called x and y of type integer, this statement will be:
            frame.foo.x = x;
            frame.foo.y = y;
        """
    ...
    def generate_return_stmt_in_new_functions(function_name):
        ...
        frame and the return value is called result, this statement will be:
