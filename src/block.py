...
class Block:
    ...
    def generate_arguments_assignments(function_info):
        ...
        For example, if the function has the declaration int foo(int x, int y), it generates these statements:
            int x = frame->foo.x;
            int y = frame->foo.y;
        So that the rest of the function's body can access these values the same as they could in
        the body of the original function which received these values as its parameters
        """
        assignments = []
    ...
    def convert_return_in_block(function_name, return_expr, func_def_map):
        ...
        if isinstance(return_expr.expr, c_ast.FuncCall):
            ...
            for i, param in enumerate(function_params):
                param_assignment = c_ast.Assignment(op='=',
                                                    ...
                                                    lvalue=utils.generate_2d_struct_ref(
                                                        ...
                                                        called_function_name, param.name, inner_ptr=True))
            ...
            return_assignment = c_ast.Assignment(op="=",
                                                 ...
                                                 lvalue=utils.generate_2d_struct_ref(
                                                     GlobalParameters.block_call_union_instance_name, function_name,
    ...
    def traverse(items, function_name, func_def_map):
        ...
        This is necessary because the return statements of the functions need to change in order to be inside
        block function. So each statement that has a block scope (E.g., switch case, if, while, for, etc.) is
        recursively checked whether it contains a return statement so that it is changed properly
        """
        new_items = []
        for item in items:
            if isinstance(item, c_ast.Return):
                ...
                if item.iffalse is not None:
                    if isinstance(item.iffalse, c_ast.If):
                        ...
                        item_clone.iffalse.block_items = Block.traverse(item.iffalse.block_items, function_name,
                                                                        func_def_map)
    ...
    def generate_function_case_body_in_block(function_info, func_def_map):
        ...
        generate_function_case_in_block wraps a case statement around the result of this function.
        """
